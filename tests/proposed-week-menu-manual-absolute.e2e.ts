...
test('allows saving a manual proposed week day even when the catalog is empty', async ({ page, request }) => {
	const backendBaseUrl =
		process.env.MOCK_BACKEND_URL || `http://127.0.0.1:${process.env.MOCK_BACKEND_PORT || 4010}`;
	await request.post(`${backendBaseUrl}/__reset`);
	...
	await page.getByTestId('week-day-action-2026-06-15').click();
	await page.getByRole('button', { name: 'Manual' }).click();
	await page.getByTestId('week-product-name-0-0').fill('Solo manual');
	await page.getByTestId('week-product-calories-0-0').fill('120');
	await page.getByTestId('week-product-carbohydrates-0-0').fill('12');
	await page.getByTestId('week-product-proteins-0-0').fill('8');
	await page.getByTestId('week-product-fats-0-0').fill('4');
